...
DrawCommand::DrawCommand(OccWidget* device, QObject *parent)
    ...
    , _device(device)
{
    if(!_device->getViewMode()){
        //aDir = _device->GetView()->Camera()->Direction().Reversed();
    }
    /* Your code here */
    // 获取鼠标左键点击信号，画图
    ...
    // 获取停止画图信号
    connect(_device, &OccWidget::endAction, this, &DrawCommand::onDeviceCancel);
}
...
void DrawCircCommand::dealWithPick(int px, int py)
{
    ...
    if(_p1_init) {
        ...
        _p1 = p;
        _p1_init = true;
        _circ = ModelMaker::make_circle(_p1, 0.1, _device->getViewMode());
...
void DrawCircCommand::dealWithPreview(int px, int py)
{
    ...
    _circ = ModelMaker::make_circle(_p1, p.Distance(_p1), _device->getViewMode());
    _device->GetContext()->Display(_circ, true);
}
...
void DrawElipsCommand::dealWithPick(int px, int py)
{
    ...
    if(_p1_init) {
        ...
        _p1 = p;
        _p1_init = true;
        _elips = ModelMaker::make_elips(_p1, 0.1, _device->getViewMode());
...
void DrawElipsCommand::dealWithPreview(int px, int py)
{
    ...
    _elips = ModelMaker::make_elips(_p1, p.Distance(_p1), _device->getViewMode());
    _device->GetContext()->Display(_elips, true);
}
...
void DrawSplineCommand::dealWithPick(int px, int py)
{
    ...
    _spline = ModelMaker::make_BSpline(_points);
...
void DrawSplineCommand::dealWithPreview(int px, int py)
{
    ...
    tmp.append(p);
    _device->GetContext()->Remove(_spline, true);
    _spline = ModelMaker::make_BSpline(tmp);;
    _device->GetContext()->Display(_spline, true);
}
...
void DrawSplineCommand::dealWithEnd(int px, int py)
{
    ...
    if(_p_init) {
        _device->GetContext()->Remove(_spline, true);
        if(_points.size() > 1) {
            _spline_final = ModelMaker::make_BSpline(_points);
